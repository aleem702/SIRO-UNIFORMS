...
import { ChevronLeft, ChevronRight, RotateCcw, ArrowRightLeft, Play, Pause, Maximize2, Menu, X } from 'lucide-react';
...
const ImageSequenceViewer = ({ basePath, frameCount, isActive, isMobile }) => {
  ...
  useAnimationFrame(() => {
    ...
    if (isAutoPlaying && zoomScale.get() < 1.05) {
  ...
  const handleDrag = (event, info) => {
    ...
    if (scale > 1.1) {
      ...
      const limitX = (rect.width * scale - rect.width) / 1.1;
      const limitY = (rect.height * scale - rect.height) / 1.1;
  ...
  const handleWheel = (e) => {
    ...
    if (newScale !== oldScale) {
      ...
      const limitX = (rect.width * newScale - rect.width) / 1.1;
      const limitY = (rect.height * newScale - rect.height) / 1.1;
    ...
    >
      ...
      {!isLoading && (
        <motion.div
          ...
          style={{
            position: 'absolute',
            // Responsive positioning for the control pillar
            right: isMobile ? '20px' : '-82px',
            top: isMobile ? 'auto' : 'calc(50% - 44px)',
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        backgroundImage: `
          radial-gradient(circle at 50% 50%, var(--surface-2) 0%, var(--surface) 100%),
    ...
    >
      ...
      <div
        ref={containerRef}
        style={{
          width: isMobile ? '100%' : 'calc(100% - 480px)', 
          flex: 1,
          position: 'relative',
          perspective: '1500px',
          display: 'flex',
      ...
      <div style={{
        ...
        >
          <ChevronLeft size={isMobile ? 32 : 48} strokeWidth={2.5} />
        </button>
        <button
        ...
        >
          <ChevronRight size={isMobile ? 32 : 48} strokeWidth={2.5} />
        </button>

      </div>

      {/* ARCHIVE INDEX: RIGHT-SIDE MASTER CONTROL (Drawer on Mobile) */}
      <motion.div 
        animate={{ 
          y: isMobile ? (isDrawerOpen ? 0 : 'calc(100% - 100px)') : 0,
        ...
        style={{
          ...
          bottom: isMobile ? '0' : '240px', 
          width: isMobile ? '100%' : '400px', 
          backgroundColor: isMobile ? '#f8f8f8' : 'rgba(255, 255, 255, 0.95)',
          backdropFilter: 'blur(60px) saturate(200%)',
          border: '1px solid rgba(255, 255, 255, 0.6)',
          borderRadius: isMobile ? '32px 32px 0 0' : '32px',
          boxShadow: isMobile ? '0 -10px 40px rgba(0,0,0,0.08)' : '0 -20px 80px rgba(0,0,0,0.1), inset 0 0 60px rgba(255,255,255,0.4)',
          display: 'flex',
          flexDirection: 'column',
          zIndex: 200,
        ...
        {isMobile && (
          <motion.div 
            ...
            style={{ 
              ...
              justifyContent: 'center', 
              cursor: 'pointer',
              borderBottom: isDrawerOpen ? '1px solid rgba(0,0,0,0.05)' : 'none',
              touchAction: 'none' // Prevent scroll interference
            }}
          >
            <div style={{ width: '40px', height: '4px', background: 'rgba(0,0,0,0.1)', borderRadius: '2px', position: 'absolute', top: '12px' }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '0 24px', width: '100%', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ background: 'var(--brand)', color: 'white', padding: '8px', borderRadius: '12px' }}>
                  <Menu size={18} />
                </div>
                <div>
                  <div style={{ fontSize: '0.6rem', fontWeight: 900, letterSpacing: '2px', color: '#888' }}>ARCHIVE</div>
                  <div style={{ fontSize: '0.9rem', fontWeight: 700 }}>{filteredDesigns[activeIndex]?.name}</div>
                </div>
              </div>
              <ChevronRight 
                size={20} 
              ...
              {filteredDesigns.map((d, idx) => (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: activeIndex === idx ? '#ffffff' : (isMobile ? 'rgba(0,0,0,0.03)' : 'rgba(255, 255, 255, 0.4)'),
      ...
      {/* CONTROL PILL - SELECTORS (Desktop Only) */}
      {!isMobile && (
        <div style={{
          position: 'absolute',
          right: '40px',
